# Step 6 ΓÇö GitHub App registration
...
**GitHub.com:** [Settings ΓåÆ Developer settings ΓåÆ GitHub Apps ΓåÆ New GitHub App](https://github.com/settings/apps/new)
...
| **Webhook** | Active |
| **Webhook URL** | `https://<tunnel-host>/api/webhooks/github` (see ┬º3) |
| **Webhook secret** | Generate a long random string ΓåÆ save as `GITHUB_WEBHOOK_SECRET` |
...
- **Any account** ΓÇö easiest for dev
- Or **Only on this account** ΓÇö if testing on your user/org only
...
1. **App ID** ΓåÆ `GITHUB_APP_ID` (numeric)
2. **Generate a private key** (.pem) ΓåÆ `GITHUB_APP_PRIVATE_KEY`
3. **Webhook secret** (from step 1) ΓåÆ `GITHUB_WEBHOOK_SECRET`

Optional for links/debugging:

- **Client ID / Client Secret** ΓÇö only if you later add ΓÇ£Install AppΓÇ¥ OAuth flow in the dashboard (not required for Step 6ΓÇô7)
...
**PowerShell** ΓÇö encode a downloaded `.pem` file:
...
1. App settings ΓåÆ **Install App**
2. Choose your user or org ΓåÆ **Only select repositories** ΓåÆ pick a test repo
3. Confirm install

You should see an **installation** webhook delivery in the AppΓÇÖs **Advanced ΓåÆ Recent deliveries** (after Step 8 handles the event; for Step 6, a manual ΓÇ£RedeliverΓÇ¥ may 200 once the app is running).
...
With `pnpm dev` and tunnel running, open App ΓåÆ **Advanced** ΓåÆ **Recent deliveries** ΓåÆ **Redeliver** a test ping if available. A `200` response confirms reachability (full event handling comes in Step 8).

## 7. OAuth vs App (what stays)

| Mechanism | Purpose |
|-----------|---------|
| **GitHub OAuth** (`GITHUB_CLIENT_ID/SECRET`) | Dashboard login, repo connect UI ΓÇö keep |
| **GitHub App** (`GITHUB_APP_*`) | Bot reviews, check runs, repo clone for indexing ΓÇö Steps 7ΓÇô10 |
...
Next: **Step 7** ΓÇö `github-app.ts` + live API smoke test with installation token.
