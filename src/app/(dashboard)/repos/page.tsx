...
type ConnectedRepo = RouterOutputs["repository"]["list"][number];
...
export default function ReposPage() {
  ...
  const connectedIds = new Set(
    (connectedRepos.data ?? []).map((repo: ConnectedRepo) => repo.githubId),
  );
      ...
      <div className="space-y-4">
        ...
        {connectedRepos.isLoading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {connectedRepos.data?.map((repo: ConnectedRepo) => (
              <ConnectedRepoCard
                key={repo.id}
                repo={repo}
