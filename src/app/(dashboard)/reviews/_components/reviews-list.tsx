...
type ReviewListItem = RouterOutputs["review"]["list"][number];
        ...
        <div className="space-y-3">
          {filteredReviews?.map((review) => (
            <ReviewCard
              ...
              retrying={triggerReview.isPending}
            />
          ))}
        </div>
      )}
...
function StatusIcon({
  ...
  switch (status) {
    ...
    case "PENDING":
      return (
        <Clock className={cn("text-amber-600 dark:text-amber-400", className)} />
      );
    case "FAILED":
      return (
...
function RiskScoreBadge({ score }: { score: number }) {
  ...
  return (
    ...
    >
      <span className={cn("size-2 rounded-full", config.barColor)} />
      {config.label}
